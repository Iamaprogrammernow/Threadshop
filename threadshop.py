...
for transaction in daily_transactions_split:
  ...
  for data_point in transaction:
    cleaned_point = data_point.replace('\n', '').strip()
    transaction_clean.append(cleaned_point)
...
total_sales = 0
for sale in sales:
  corrected_price = sale.strip('$')
  total_sales += float(corrected_price)
#print(total_sales)
...
for item in thread_sold:
  if addsign not in item:
    thread_sold_split.append(item)
  else:
    for split_item in item.split('&'):
      thread_sold_split.append(split_item)
...
for item in color:
  print('Thread Shed sold {} threads of {} thread today'.format(color_count(item), item))
